...
def render_main_layout():
    ...
    # --- Create 3 columns: Left (Controls), Center (Compose), Right (Monitor)
    col1, col2, col3 = st.columns([1, 2, 1], gap="medium")

    # --- Left Column: Controls (Commit, Branch, Guide)
    with col1:
        render_controls_panel()

    # --- Center Column: Compose (YAML, Draft, Preview)
    with col2:
...
import uuid
refresh_key = f"refresh_feed_main_{st.session_state.get('active_tab', 'default')}_{uuid.uuid4().hex[:8]}"
