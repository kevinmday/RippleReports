...
with tab_compose:
    ...
    st.caption("Choose whether to stay and refine your YAML, or move straight into writing.")

    # --- Save YAML (stay on Compose) ---
    if st.button("💾 Save YAML (Stay on Compose)"):
        save_yaml(title, author, date, format_choice, intention_equation)
        st.success("✅ YAML saved. You can continue editing before moving on.")

    # --- Save & Move to Input ---
    if st.button("🚀 Save & Move to Input"):
        save_yaml(title, author, date, format_choice, intention_equation)
        st.session_state["active_tab"] = "Input"
        st.success("✅ YAML saved and scaffold created. Moving to Input...")
