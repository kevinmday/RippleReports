...
def render_right_sidebar(tab_name="main"):
    ...
    with st.container(border=True, height=250):
        ...
        # ⬅️ key now includes tab_name
        st.button("🔄 Refresh Feed", key=f"refresh_feed_sidebar_{tab_name}")
...
with colL:
    ...
    intention_options = [
        "None",
        "Clarity x Credibility - Evidence",
    ...
    if choice == "(new)":
        new_name = st.text_input("Choose File Name", value="new-article.yaml")
        ...
        with st.container():
            ...
            # tighter control of vertical space
            st.markdown("<div style='margin-top:-35px'></div>", unsafe_allow_html=True)

            col1, col2 = st.columns([1, 1])
            with col1:
                st.button("Write Now with AI Assistant", key="write_ai_now")
            with col2:
                st.button("Write Now with Intention!", key="write_intent_now")
        ...
        st.markdown(
            """
            <style>
            /* Reduce vertical space *after* the intention equation section only */
            div[data-testid="stVerticalBlock"] div[data-testid="stSelectbox"]:last-of-type {
                margin-bottom: -35px;
            }
            </style>
            """,
        ...
        st.markdown(
            """
            <style>
            /* Target the block that contains the Write Now buttons */
            div[data-testid="stVerticalBlock"] div:has(> button[kind="primary"]) {
                margin-top: -140px !important;
            }

            /* Keep spacing between the two Write buttons consistent */
            button[kind="primary"] + button[kind="primary"] {
                margin-top: 10px !important;
            }

            /* Optionally bring the "Write Full Article Now" header closer */
            h2:contains("Write Full Article Now") {
                margin-top: -10px !important;
            }
            </style>
            """,
        ...
        st.markdown(
            """
            <style>
            /* Pull both Write buttons closer to the equation dropdown */
            button[kind="primary"]:nth-of-type(1) {
                margin-top: -80px !important;
            }
            /* Tighten gap between the two write buttons */
            button[kind="primary"]:nth-of-type(2) {
                margin-top: 5px !important;
            }
            /* Pull header a little tighter */
            h2:contains("Write Full Article Now") {
                margin-top: -10px !important;
            }
            </style>
...
with st.container():
    ...
    col1, col2 = st.columns([1, 1])
    with col1:
        st.button("Write Now with AI Assistant", key="write_ai_now_compose")
    with col2:
        st.button("Write Now with Intention!", key="write_intent_now_compose")

# Right-hand column (status + monitors)
render_right_sidebar("compose")

create_ai_btn = st.button("Write Now with AI Assistant", key="create_ai_draft")
...
if create_ai_btn:
    try:
        ...
        if p.exists():
            ...
            # Step 1: Create base YAML draft
            payload = default_article()
            payload["format"] = new_format
            payload["intention_equation"] = new_equation
            save_yaml(p, payload)

            # Step 2: Generate sections using your LLM helper
            sections = generate_sections_with_llm(payload)
    ...
    with colR:
        ...
        # Define layout columns (so col_right exists)
        col_left, col_right = st.columns([2, 3])

        with col_right:  # keep existing layout context
            st.markdown(
                """
                <div style="
                    text-align:right;
    ...
    yaml_valid = True
    missing_fields = ["audience"]  # placeholder example
...
st.markdown("### Write Full Article Now")

# Simplified: remove duplicate selectboxes, keep only the render button
if st.button("Write Now with Intention!", key="rw_write_render", disabled=(choice == "(new)")):
    try:
