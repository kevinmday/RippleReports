...
tab_compose, tab_source, tab_meta, tab_preview = st.tabs(
    ["Compose", "Prompt", "Analyze", "Preview"]
)
# -------------------------
# Tab 1: Compose (YAML)
# -------------------------
...
with tab_compose:
    ...
    with colR:
        ...
        with col_right:  # keep existing layout context
            st.markdown("?? **Sanity Check Feedback**")

            # --- Gather session / file info ---
            current_draft = st.session_state.get(CURRENT_DRAFT_KEY, "(new)")
            draft_path = OUTPUT_DIR / f"{current_draft}.yaml"
            last_modified = (
            time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(draft_path.stat().st_mtime))
            if draft_path.exists() else "Not saved yet"
            )
